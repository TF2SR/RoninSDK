...
struct PakLoadFuncs
{
	...
	__int64 qwordB0;
	__int64 qwordBB;
	void* (*OpenFile)(const char* pPath, void* pCallback);
	__int64 CloseFile;
	__int64 qwordD0;
...
// only used in rpakfilesystem.cpp so we have it here
...
struct ModPak_t
{
	//std::string m_modName; // unnecessary, no mod support in ronin
...
public:
	...
	void AddModPak(ModPak_t modPak)
	{
		m_modPaks.push_back(modPak);
	}
...
struct VRPakFileSystem : public IDetour
{
