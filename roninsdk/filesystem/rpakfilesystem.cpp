...
void PakLoadManager::UnloadModPaks()
{
	...
	assert(m_mapPaks.size() == 0, "Not all map paks were unloaded?");
}
...
{
	return g_pakLoadApi->OpenFile(path, nullptr);
}
...
inline PakHandle (__fastcall *LoadPakAsync)(const char* pPath, void* memoryAllocator, int flags);
// clang-format off
//HOOK(LoadPakAsyncHook, LoadPakAsync,
//	PakHandle, __fastcall, (const char* pPath, void* memoryAllocator, int flags))
PakHandle __fastcall LoadPakAsyncHook(const char* pPath, void* memoryAllocator, int flags)
	// clang-format on
{
...
inline void* (__fastcall *UnloadPak)(PakHandle nPakHandle, void* pCallback);
// clang-format off
//HOOK(UnloadPakHook, UnloadPak,
//	void*, __fastcall, (PakHandle nPakHandle, void* pCallback))
void* __fastcall UnloadPakHook(PakHandle nPakHandle, void* pCallback)
	// clang-format on
{
...
inline void* (__fastcall *o_pOpenFile)(const char* pPath, void* pCallback);
// clang-format off
//HOOK(OpenFileHook, o_pOpenFile,
//	void*, __fastcall, (const char* pPath, void* pCallback))
void* __fastcall OpenFileHook(const char* pPath, void* pCallback)
	// clang-format on
{
...
void __fastcall v_Pak_Free(PakLoadedInfo_s* info)
{
	if (info->pakFile)
	{
...
void VRPakFileSystem::GetFun(void) const
{
	g_pPakLoadManager = new PakLoadManager;

	if (fs::exists(".\\ronin\\paks\\ronin.rpak")) // custom ronin rpak
	{
		ModPak_t roninRpak = ModPak_t();
		roninRpak.m_path = "./ronin/paks/ronin.rpak";
		roninRpak.m_pathHash = STR_HASH(roninRpak.m_path);
		roninRpak.m_preload = true;
		g_pPakLoadManager->AddModPak(roninRpak);
	}

	g_pakLoadApi = g_pEngineDll->Offset(0x5BED78).Deref().RCast<PakLoadFuncs*>();

	LoadPakAsync = g_pakLoadApi->LoadRpakFileAsync;
	UnloadPak = g_pakLoadApi->UnloadPak;
	o_pOpenFile = g_pakLoadApi->OpenFile;
	pszCurrentMapRpakPath = g_pEngineDll->Offset(0x1315C3E0).RCast<decltype(pszCurrentMapRpakPath)>();
	piCurrentMapRpakHandle = g_pEngineDll->Offset(0x7CB5A0).RCast<decltype(piCurrentMapRpakHandle)>();
	piCurrentMapPatchRpakHandle = g_pEngineDll->Offset(0x7CB5A4).RCast<decltype(piCurrentMapPatchRpakHandle)>();
	ppModelLoader = g_pEngineDll->Offset(0x7C4AC0).RCast<decltype(ppModelLoader)>();
	rpakMemoryAllocator = g_pEngineDll->Offset(0x7C5E20).RCast<decltype(rpakMemoryAllocator)>();

	o_pLoadGametypeSpecificRpaks = g_pEngineDll->Offset(0x15AD20).RCast<decltype(o_pLoadGametypeSpecificRpaks)>();
	o_pCleanMaterialSystemStuff = g_pEngineDll->Offset(0x12A11F00).RCast<decltype(o_pCleanMaterialSystemStuff)>();
	o_pCModelLoader_UnreferenceAllModels = g_pEngineDll->Offset(0x5ED580).RCast<decltype(o_pCModelLoader_UnreferenceAllModels)>();
	o_pLoadlevelLoadscreen = g_pEngineDll->Offset(0x15A810).RCast<decltype(o_pLoadlevelLoadscreen)>();

	o_pLoadMapRpaks = g_pEngineDll->Offset(0x15A8C0).RCast<decltype(o_pLoadMapRpaks)>();
...
void VRPakFileSystem::Attach(void) const
{
	DetourAttach((PVOID*)(&o_pLoadMapRpaks), h_LoadMapRpaks);
	DetourAttach((LPVOID*)(&Pak_Free), v_Pak_Free);

	DetourAttach((PVOID*)(&LoadPakAsync), LoadPakAsyncHook);
	DetourAttach((PVOID*)(&UnloadPak), UnloadPakHook);
	DetourAttach((PVOID*)(&o_pOpenFile), OpenFileHook);
}
void VRPakFileSystem::Detach(void) const
{
}
